...
(
    id                 BIGSERIAL PRIMARY KEY,
    package_version_id BIGINT NOT NULL REFERENCES package_versions (id),
    namespace          TEXT,  -- A namespace can be null in C (dummy modules)
    created_at         TIMESTAMP,
    metadata           JSONB
);
...
-- In C we have dummy modules, in general we do not use the modules table.
--  CREATE UNIQUE INDEX CONCURRENTLY unique_version_namespace ON modules USING btree (package_version_id);
--  ALTER TABLE modules
...
-- In C this constrain does not exist because we have external static functions.
--  ALTER TABLE callables
