...
//                List<ExtendedRevisionCallGraphV3> resolvedCallGraphs =
//                mergeCallGraphs(revisionCallGraphs);
    ...
    private static RevisionCallGraph mergeWithCHA(RevisionCallGraph artifact,
        ...
        for (final var arc : artifact.getGraph().getExternalCalls().entrySet()) {
            ...
            if (Integer.parseInt(arc.getValue().getOrDefault("invokevirtual", "0")) > 0 ||
                    Integer.parseInt(arc.getValue().getOrDefault("invokeinterface", "0")) > 0) {
